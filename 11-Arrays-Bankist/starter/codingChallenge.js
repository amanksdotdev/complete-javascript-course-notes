...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  const allDogs = dogsJuliaCopy.concat(dogsKateCopy);
  console.log(allDogs);
...
//2
const eatingCheck = function (currFood, recFood) {
  if (currFood < recFood - recFood * 0.1) {
    return 'little';
  } else if (currFood > recFood + recFood * 0.1) {
    return 'much';
  }
...
console.log(
  `Sarah's Dog is eating too ${eatingCheck(
    sarahDog.curFood,
...
const ownersEatTooMuch = dogs
  .filter(obj => obj.curFood > obj.recommendedFood + obj.recommendedFood * 0.1)
  .map(obj => obj.owners)
  .flat();
...
//6
const checkEatingRecommended = function (dog) {
  return (
...
console.log(dogs.some(checkEatingRecommended));

//7
const eatingOkay = dogs.filter(checkEatingRecommended);
